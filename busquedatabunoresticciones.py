...
def generate_neighbors(G, current_edges):
    """Generates neighbors by swapping edges with the given restrictions."""
    all_edges = list(G.edges)
    neighbors = []
    
    for edge_to_remove in current_edges:
        for edge_to_add in all_edges:
            # Restricción 2: No permitir más de una de las ligaduras AD, CD o AB
            if edge_to_add not in current_edges and edge_to_add not in current_edges:
                if edge_to_add in [('A', 'D'), ('C', 'D'), ('A', 'B')] and sum(1 for e in current_edges if e in [('A', 'D'), ('C', 'D'), ('A', 'B')]) >= 1:
                    continue
            
            # Restricción 1: Si se incluye la ligadura AD, también debe incluirse DE
            if ('A', 'D') in current_edges and ('D', 'E') not in current_edges:
                continue
            if ('D', 'E') in current_edges and ('A', 'D') not in current_edges:
                continue
